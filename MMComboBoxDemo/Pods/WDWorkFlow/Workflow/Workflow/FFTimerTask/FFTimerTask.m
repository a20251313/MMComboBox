...
+ (void)countDownTaskWithTarget:(id)target
    ...
    [FFSystemTimeManager sharedInstance].systemTime = serverTime;
    [[FFSystemTimeManager sharedInstance] addObserverForSystemTime:target onChanged:^(id target, id observer, id value, NSDictionary *change) {
        ...
        if (remainSecond > 0 && mode != CountDownModeFinished) {
            ...
            // 倒计时结束，移除kvo
            [[FFSystemTimeManager sharedInstance] removeObserverForSystemTime:target];
        }
        if (handler) {
